...
const DetailService = {
  ...
  trackDetailCreate: (detailData: TrackDetailData, trackId: number) => {
    return api
      .post(`/track/${trackId}/detail`, detailData, {
        headers: { ...authHeader() },
      })
      .then((response) => response.data.data)
      .catch((er) => catchError(er));
  ...
  trackDetailUpdate: (detail: TrackDetailData, detailId: number) => {
    return api
      .put(`/track/detail/${detailId}`, detail, {
        headers: { ...authHeader() },
      })
      .catch((er) => catchError(er));
  },
  ...
  trackDetailCoursesGet: (searchQuery: string) => {
    return api
      .get(`/search/courses?q=${searchQuery}`, {
        headers: { ...authHeader() },
      })
  ...
  trackDetailEventsGet: (searchQuery: string) => {
    return api
      .get(`/search/events?q=${searchQuery}`, {
        headers: { ...authHeader() },
      })
      .then((response) => response.data)
      .catch((er) => catchError(er));
