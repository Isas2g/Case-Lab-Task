...
export const DetailUpdateModal = observer(({show, onHide, type, mutated, setMutated, trackDetail}: Props): JSX.Element => {
    ...
    return <ModalComponent title="Изменить элемент трека" heading="" show={show} onHide={onHide} remove={false} track={undefined}>
        <input type="text" onChange={({target}) => setSearchQuery(target.value)} />
        
        {searches.map((item: Course | Event) => 
          <p className={style.searchItem} onClick={() => changeEntity(item.id)}>{item.id} {item.name}</p>
        )}
