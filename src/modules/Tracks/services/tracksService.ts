...
const TrackService = {
  ...
  trackPreview: async (fileData: any) => {
    ...
    return await api
      .post("/track/preview", formData, {
        headers: { ...authHeader() },
      })
      .then((response) => response.data.data.file.url)
      .catch((er) => catchError(er));
  },
  ...
  trackCreate: (data: TrackData) => {
    api
      ...
      .then((response) => console.log(`${response.data.message} track "${data.name}"`))
      .catch((er) => catchError(er));
  },

  trackUpdate: (track: Track) => {
    api
      .put(`/track/${track.id}`, track.data, {
        headers: { ...authHeader() },
      })
      .catch((er) => catchError(er));
  },
